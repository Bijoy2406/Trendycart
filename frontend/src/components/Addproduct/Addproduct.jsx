...
const Addproduct = () => {
  ...
  const handleSubmit = async () => {
    ...
    const url = product.id
      ? 'https://backend-beryl-nu-15.vercel.app/updateproduct'
      : 'https://backend-beryl-nu-15.vercel.app/addproduct';

    await fetch(url, requestOptions).then((resp) => resp.json()).then((data) => {
      if (data.success) {
        product.id ? updateProduct(product) : addProduct(product);
        alert(`Product ${product.id ? 'updated' : 'added'} successfully`);
        navigate('/admin');
      } else {
        alert(`Failed to ${product.id ? 'update' : 'add'} product`);
      }
    });
  };
